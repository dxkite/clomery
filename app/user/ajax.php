...
class ajax
{
    ...
    public function main()
    {
        ...
        if (isset($json['type'])) {
            switch ($json['type']) {
                ...
                case 'checkuser':
                if (isset($json['user'])) {
                    return ['return'=>DB_User::userExist($json['user'])];
                }break;
                 case 'checkemail':
                if (isset($json['email'])) {
                    return ['return'=>DB_User::emailExist($json['email'])];
                }break;
                case 'signup':
    ...
    {   
        ...
        if (strtoupper(Session::get('verify_code'))!==strtoupper($code)) {
            ...
            if ($id=DB_User::signup($user, $passwd, $email)) {
    ...
    public function signin(string $name, string $passwd,string $keep)
    {
        if (!preg_match(self::REG_UNAME, $name)) {
            ...
            if (($rt=DB_User::signin($name, $passwd,$keep))===0) {
                return ['return'=>true];
            }
