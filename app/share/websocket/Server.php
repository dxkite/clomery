...
class Server
{
    ...
    public  function listen(int $port=0)
    {
        ...
        while (true) {
            ...
            if (in_array($this->socket, $readable)) {
                ...
                $header = self::read($socket_new, 1024);
            ...
            foreach ($readable as $id => $client_readable) {
                ...
                //检查offline的client
                $buf = self::read($client_readable, 1024, PHP_NORMAL_READ);
                if ($buf === false) {
    ...
    {
        $this->callers[]=new \suda\tool\Command($callback);
    }
    // 发送消息到用户
    public  function pushMessage(string $message, int $client_id=-1)
    {
        ...
        if ($client_id<0) {
            foreach ($this->clients as $key=> $client) {
                ...
                self::write($client, $msg, strlen($msg));
            }
        } else {
            self::write($clients[$client_id], $msg, strlen($msg));
        }
    }
    ...
    public function perform_handshaking($receved_header, $client_conn, $host, $port)
    {
        ...
        // var_dump($headers['Sec-WebSocket-Key']);
    ...
    public static function write($socket,string $string,int $length){
        set_error_handler(['websocket\\Server','errorHander']);
        $return=socket_write($socket,$string, $length);
        restore_error_handler();
        return $return;
    }
    public static function read( $socket , int $length  , int $type = PHP_BINARY_READ  ){
        set_error_handler(['websocket\\Server','errorHander']);
        $return=socket_read($socket,$length, $type);
        restore_error_handler();
        return $return;
    }
    public static function errorHander(int $errno, string $errstr, string $errfile, int $errline, array $errcontext)
    {
        echo "error[{$errno}] info {$errfile}:{$errline}>".$errstr."\r\n";
    }
}
