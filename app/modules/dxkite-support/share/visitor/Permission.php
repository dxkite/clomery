 ...
 * 二级权限验证，支持忽略父级元素
 */
class Permission implements \JsonSerializable, MethodParameter
{
    // 权限表,包含所有的权限结构
    ...
    public function jsonSerialize()
    {
        ...
        foreach ($this->toArray() as $value) {
            $permissions[$value] = self::alias($value);
        }
        return $permissions;
    }
    ...
    public static function readPermissions()
    {
        ...
        foreach ($modules as $module) {
            $tmp=  app()->getModuleConfig($module, 'permissions');
            if (is_array($tmp)) {
    ...
    {
        return new Permission($jsonData);
    }
